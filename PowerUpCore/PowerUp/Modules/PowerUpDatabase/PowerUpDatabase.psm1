...
<#
Add-Type -TypeDefinition "public enum IfDatabaseObjectExists { Error, ReturnExisting }" 

function New-SqlServerDatabase(
    [Parameter(Mandatory=$true)] [string] $name,
    [string] $server = '.',
    [IfDatabaseObjectExists] $ifExists = 'Error'
) {
    Import-Module PowerUpUtilities

    [System.Reflection.Assembly]::LoadWithPartialName('Microsoft.SqlServer.SMO') | Out-Null
    Use-Object (New-Object Microsoft.SqlServer.Management.Smo.Server($server)) {
        param ($serverSmo)

        if ($serverSmo.Databases.Contains($name)) {
            if ($ifExists -eq IfDatabaseObjectExists::Error) {
                throw "Database '$name' already exists on '$server'"
            }
            
            Write-Host "Database '$name' already exists on '$server', skipping."
            $database = $serverSmo.Databases[$name]
            return
        }
...
function New-SqlServerLogin(
    [Parameter(Mandatory=$true)] [string] $name,
    [string] $server = '.',
    [IfDatabaseExists] $ifExists = 'Error'
) {
    Import-Module PowerUpUtilities
    ...
    Use-Object (New-Object Microsoft.SqlServer.Management.Smo.Server($server)) {
        ...
        if ($serverSmo.Logins.Contains($name)) {
            if ($ifExists -eq IfDatabaseObjectExists::Error) {
                throw "Login '$name' already exists on '$server'"
            }
            
            Write-Host "Login '$name' already exists on '$server', skipping."
